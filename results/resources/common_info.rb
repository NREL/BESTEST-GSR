module BestestResults
  ...
  def self.populate_common_info(program = "EP")
    ...
    if program == "OS"
      ...
      hash[:program_version_release_date] = "11/14/2024 (expected)"
      hash[:program_name_short] = "OS"
    else
      hash[:program_name_and_version] = "EnergyPlus 24.2.1"
      hash[:program_version_release_date] = "10/03/24"
      hash[:program_name_short] = "E+"
    end

    hash[:results_submission_date] = "11/15/2024"
